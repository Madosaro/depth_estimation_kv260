...
class MDELoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.register_buffer('kernel', self._create_gaussian_kernel(7, 1.5))
    ...
    def forward(self, y_pred, y_true, w1=1.0, w2=1.0, w3=1.0, w4=1.0, return_components=False):
        ...
        l1_depth = torch.sum(torch.abs(y_pred - y_true) * mask) / (mask.sum() + 1e-8)   #MAE (Mean Absolute Error)
        l2_depth = torch.sum(torch.pow(y_pred - y_true, 2) * mask) / (mask.sum() + 1e-8)    #MSE (Mean Square Error)
        ...
        if return_components:
            return total_loss, l_ssim, l_edges, l1_depth, l2_depth
